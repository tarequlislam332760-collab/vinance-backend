...
/* Generic apply (old route kept for compatibility) */
app.post('/api/capital/apply', auth, async (req, res) => {
  try {
    const { fundName, amount, type, message } = req.body;
    const a = await CapitalApp.create({
      userId: req.user._id, name: req.user.name, email: req.user.email,
      fundName, amount, type: type || 'fund', message,
    });
    res.json({ message:'Application submitted!', application:a });
  } catch (e) { res.status(500).json({ message:e.message }); }
});
...
app.post('/api/capital/apply-fund', auth, async (req, res) => {
  try {
    ...
    const existing = await CapitalApp.findOne({ userId:req.user._id, targetName, type:'fund_invest', status:'pending' });
    if (existing) return res.status(400).json({ message:'You already have a pending application for this fund' });
    const a = await CapitalApp.create({
      userId:     req.user._id,
      name:       req.user.name,
    ...
    res.json({ message:`Application submitted for ${targetName}!`, application:a });
  } catch (e) { res.status(500).json({ message:e.message }); }
});
...
app.post('/api/capital/apply-vc', auth, async (req, res) => {
  try {
    ...
    const existing = await CapitalApp.findOne({ userId:req.user._id, targetName, type:'vc_apply', status:'pending' });
    if (existing) return res.status(400).json({ message:'You already have a pending application for this VC' });
    const a = await CapitalApp.create({
      userId:     req.user._id,
      name:       req.user.name,
      email:      req.user.email,
    ...
    res.json({ message:`Application sent to ${targetName}!`, application:a });
  } catch (e) { res.status(500).json({ message:e.message }); }
});
...
app.post('/api/capital/register-fund', auth, async (req, res) => {
  try {
    ...
    res.json({ message:'Fund registration submitted!', application:a });
  } catch (e) { res.status(500).json({ message:e.message }); }
});
